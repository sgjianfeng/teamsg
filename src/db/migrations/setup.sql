...
CREATE POLICY "Users can update own profile" ON users
  FOR UPDATE USING (auth.uid()::text = users.id);

DROP POLICY IF EXISTS "Users can insert own profile" ON users;
CREATE POLICY "Users can insert own profile" ON users
  FOR INSERT WITH CHECK (auth.uid()::text = id);

-- Basic Policies - Authenticated users can do basic operations
...
CREATE POLICY "Teams are manageable by authenticated users" ON teams
  FOR ALL USING (auth.role() = 'authenticated');

-- Groups Policies - Simple authenticated access
CREATE POLICY "Groups are viewable by authenticated users" ON groups
  FOR SELECT USING (auth.role() = 'authenticated');

CREATE POLICY "Groups are manageable by authenticated users" ON groups
  FOR ALL USING (auth.role() = 'authenticated');

-- Members Policies - Simple authenticated access
CREATE POLICY "Members are viewable by authenticated users" ON members
  FOR SELECT USING (auth.role() = 'authenticated');

CREATE POLICY "Members are manageable by authenticated users" ON members
  FOR ALL USING (auth.role() = 'authenticated');
